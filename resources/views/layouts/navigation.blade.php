        ...
        <div class="flex flex-wrap justify-between items-center p-4 mx-auto max-w-screen-xl">
            ...
            <div class="hidden w-full" id="navbar-dropdown">
                <ul class="flex flex-col p-4 mt-4 font-medium bg-gray-50 rounded-lg border border-gray-100 rtl:space-x-reverse dark:bg-gray-800 dark:border-gray-700">
                    ...
                    @if (Auth::user()->role == 1)
                        <li>
                            <a href="/vendor/dashboard" class="block px-3 py-2 text-black rounded hover:text-white hover:bg-secondary" aria-current="page">Dashboard Seller</a>
                        </li>
                        <li>
                            <a href="{{route('antrian')}}" class="block px-3 py-2 text-black rounded hover:text-white hover:bg-secondary">Lihat Pesanan</a>
                        </li>
                    @endif
                    @if (Auth::user()->role == 2)
                        <li>
                            <a href="/user/dashboard" class="block px-3 py-2 text-black rounded hover:text-white hover:bg-secondary" aria-current="page">Profile</a>
                        </li>
                        <li>
                            <a href="{{route('riwayat')}}" class="block px-3 py-2 text-black rounded hover:text-white hover:bg-secondary">Riwayat Pesanan</a>
                        </li>
                    @endif
                    ...
                    <li>
                        <form action="{{ route('logout') }}" method="POST">
                            @csrf
                            <button type="submit" class="block px-3 py-2 w-full text-black rounded text-start hover:text-white hover:bg-secondary">Logout</button>
                        </form>
                    </li>
                </ul>
            </div>
